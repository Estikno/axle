...
namespace Axle::Debug {
    ...
    void DebugConsole::Draw(const char* title, bool* p_open) {
        ...
        // As a specific feature guaranteed by the library, after calling Begin() the last Item represent the title
        // bar. So e.g. IsItemHovered() will return true when hovering the title bar. Here we create a context menu
        ...
        ImGui::TextWrapped("This is a debu console.");
        ...
        const float footer_height_to_reserve =
            style.SeparatorTextBorderSize + style.ItemSpacing.y + ImGui::GetFrameHeightWithSpacing();
        if (ImGui::BeginChild("ScrollingRegion",
            ...
            // clipping to only process visible items. The clipper will automatically measure the height of your
            // first item and then "seek" to display only items in the visible area. To use the clipper we can
            // replace your standard loop:
            ...
            // property. We would need random-access on the post-filtered list. A typical application wanting coarse
            // clipping and filtering may want to pre-compute an array of indices or offsets of items that passed
            // the filtering test, recomputing this array when user changes the filter, and appending newly elements
            // as they are inserted. This is left as a task to the user until we can manage to improve this example
        ...
        if (ImGui::InputText(
                "Input", InputBuf, IM_COUNTOF(InputBuf), input_text_flags, &TextEditCallbackStub, (void*) this)) {
            char* s = InputBuf;
            Strtrim(s);
            if (s[0])
                ExecCommand(s);
    ...
    void DebugConsole::ExecCommand(const char* command_line) {
        ...
        } else if (Stricmp(tokens[0].c_str(), "ENABLE_CHANNEL") == 0) {
            if (tokens.size() < 2) {
                AddLog("[error] Usage: ENABLE_CHANNEL <channel>");
                AddLog("channels: Core, Events, ...");
            } else {
        ...
        } else if (Stricmp(tokens[0].c_str(), "DISABLE_CHANNEL") == 0) {
            if (tokens.size() < 2) {
                AddLog("[error] Usage: DISABLE_CHANNEL <channel>");
                AddLog("channels: Core, Events, ...");
            } else {
                const std::string& channelStr = tokens[1];
                LogChannel channel = ChannelFromStr(channelStr);
